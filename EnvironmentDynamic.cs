...
public class EnvironmentDynamic : MonoBehaviour
{
    public InputField AutoDynamicSingle;
    public InputField AutoDynamicDouble;
    ...
    void Update()
    {
        ...
        if (TimeData > 0)
        {
            ...
            foreach (Hand hand in frame.Hands)
                if (hand.PalmVelocity.Magnitude > smallestVelocity)
            {
                num2++;
            }
            else
                ...
                if (!IsStationary(handB))
                {
                    ...
                    if ((vsd1.magnitude <= 0.15f) && EqualCode(codeEnd, codeRefer))
                    {
                        ...
                        if (!EqualCode(codeRefer, codeStart) && !EqualCode(codeRefer, codeEnd) && handWave != "W0")
                        {
                            ...
                            txRightHandDynamic = hand_codesr[1] + ">" + handresdy + ">" + hand_codesr[num2];
                            AutoDynamicSingle.text = handres5.ToString() + ">" + handresdy.ToString() + ">" + handres6.ToString();
                ...
                if (!IsStationary(handB))
                {
                    ...
                    if ((vsd1.magnitude <= 0.15f) && EqualCode(LcodeEnd, LcodeRefer))
                    {
                        ...
                        if (!EqualCode(LcodeRefer, LcodeStart) && !EqualCode(LcodeRefer, LcodeEnd) && LhandWave != "W0")
                        {
                            ...
                            AutoDynamicSingle.text = handres5.ToString() + ">" + handresdy.ToString() + ">" + handres6.ToString();
                            if (handsTouch == null) AutoDynamicDouble.text = hand_codesl[1] + ">" + handresdy + ">" + hand_codesl[num2] + "," + txRightHandDynamic;
                            else AutoDynamicDouble.text = hand_codesl[1] + ">" + handresdy + ">" + hand_codesl[num2] + "," + txRightHandDynamic + "," + handsTouch;
                            //恢复初始值
                            LhandWave = "W0";
                            LcodeStart.Clear();
                            LcodeEnd.Clear();
