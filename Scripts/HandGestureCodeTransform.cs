...
public class HandGestureCodeTransform : MonoBehaviour
{
    ...
    public InputField tx4;
    public InputField tx5;
    public InputField tx6;
    public InputField tx7;
    public InputField tx8;
    public InputField tx81;
    public InputField tx82;
    public InputField tx9;
    public InputField timetext;
    public InputField txsinglehand;
    public InputField txdoublehand;
    public InputField txLeftHandDynamic;
    public InputField txRightHandDynamic;
    ...
    const float smallestVelocity = 0.02f;
    [Tooltip("Velocity (m/s) move toward ")]
    ...
    //{
    //    File.Delete(path);
    //    if (!File.Exists(path)) // 判断原本是否存在该文件，不存在则执行新建文件，path为文件路径，例如 
    //                            // path = Application.streamingAssetsPath + “/user.csv" 
    //    {
    //        Debug.Log("新建" + path);
    ...
    void Start()
    {
        ...
        //Exist(testeename+"SingleHandData"+".csv");
        //Exist(testeename+"DoubleHandData"+".csv");
        //Exist(testeename+"DynamicHandData"+".csv");
        //画圈
    ...
    string[] hand_codes = new string[1000];
    string[] hand_codes1 = new string[1000];
    string[] hand_codesl = new string[1000];
    string[] hand_codesr = new string[1000];
    //string c;
    //int one, two;
    int num2=0;
    ...
    void Update()
    {
        ...
        if (TimeData > 6)
        {
            //tx1.text = hand_codes[1];  //输出第一个编码
            //tx8.text = hand_codes[num2];  //输出最后一个编码
            timetext.text = "停止识别";
            hand_codes1[0] = hand_codes[0];
            Notice.text = "Stop";
            ...
            {
                if (hand_codes[i] != hand_codes[i + 1])
                {
                    hand_codes1[num3] = hand_codes[i + 1];
                    num3++;
                }
                
            }

            ds = num3 / 2;  //判断编码个数是单双数

            if ((ds - (int)ds > 0))     //单数给出四个编码选择
            {
                tx4.text = hand_codes1[((num2 + 1) / 2) - 1];
                tx5.text = hand_codes1[((num2 + 1) / 2)];
                tx6.text = hand_codes1[((num2 + 1) / 2) + 1];
                tx7.text = hand_codes1[((num2 + 1) / 2) + 2];
            }
            else                 //双数给出四个编码选择
            {
                tx4.text = hand_codes1[(num2 / 2) - 1];
                tx5.text = hand_codes1[(num2 / 2)];
                tx6.text = hand_codes1[(num2 / 2) + 1];
                tx7.text = hand_codes1[(num2 / 2) + 2];
            }
        }
        if (TimeData > 3 && TimeData <= 6)
        {
            ...
            if (frame.Hands.Count == 1)
            {
                ...
                if (handres2 != handres0 && handres2 != null)
                {
                    ...
                    hand_codes[num2] = handres2;
                    tx1.text = hand_codes[1];  //单手时输出第一个编码
                ...
                if (handresdb != handres1 && handresdb != null)
                    {
                        ...
                        hand_codes[num2] = handresdb;
                        hand_codesl[num2] = handres3;
                        hand_codesr[num2] = handres4;
                        tx2.text = hand_codes[1];  //双手时输出第一个编码
                        tx21.text = hand_codesl[1];
                        tx22.text = hand_codesr[1];
                }

                }
                //CSVs.Write(doublecodes, "DoubleHandData.csv");
                        ...
                        if ((vsd1.magnitude <= 0.15f) && EqualCode(codeEnd, codeRefer))
                        {
                            ...
                            if (maxIndex != posFloat.Count && posFloat.Count >= point)
                            {
                                ...
                                if (Vector3.Distance(beginPos, endPos) <= radius && isBool)
                                {
                                    ...
                                    var perlength = perp.magnitude;
                                    perp /= perlength;
                                angp1[0] = Vector3.Angle(perp, v1);
                                angp1[1] = Vector3.Angle(perp, v2);
                                angp1[2] = Vector3.Angle(perp, v3);
                                angp1[3] = Vector3.Angle(perp, v4);
                                angp1[4] = Vector3.Angle(perp, v5);
                                angp1[5] = Vector3.Angle(perp, v6);
                                int num1 = Min(angp1, 6);
                                // 画了一个圆
                                handWave = "C" + num1.ToString();
                                Debug.Log("圆");
                            ...
                            tx3.text = handres5.ToString() + ">" + handresdy + ">" + handres6.ToString();
                            txRightHandDynamic.text= hand_codesr[1] + ">" + handresdy + ">" + hand_codesr[num2];
                    ...
                    if ((vsd1.magnitude <= 0.15f) && EqualCode(LcodeEnd, LcodeRefer))
                    {
                        ...
                        if (LmaxIndex != LposFloat.Count && LposFloat.Count >= point)
                        {
                            ...
                            if (Vector3.Distance(LbeginPos, LendPos) <= Lradius && LisBool)
                            {
                                ...
                                if (handB.IsLeft == false)
                                {
                                    LhandWave = "C" + num1.ToString();//+ "," + Lradius.ToString() + "," + perp.ToString() + "," + "Right";
                                }
                                else
                                {
                                    LhandWave = "C" + num1.ToString();//+ "," + Lradius.ToString() + "," + perp.ToString() + "," + "Left";
                                }
                                Debug.Log("圆");
                                Debug.Log(perp.ToString());
                            }
                        ...
                        if (!EqualCode(LcodeRefer, LcodeStart) && !EqualCode(LcodeRefer, LcodeEnd) && LhandWave != "W0")
                        {
                            ...
                            tx3.text = handres5.ToString() + ">" + handresdy + ">" + handres6.ToString();
                            txLeftHandDynamic.text = hand_codesl[1] + ">" + handresdy + ">" + hand_codesl[num2];
                            dynamiccodes.Add(handresdy + "," + LDynamicStart_Time + "," + TimeData);
                            txsinglehand.text = handres5.ToString() + ">" + LhandWave.ToString() + ">" + handres6.ToString();
                            if (handsTouch == null) txdoublehand.text = txLeftHandDynamic.text + "," + txRightHandDynamic.text;
                            else txdoublehand.text = txLeftHandDynamic.text + "," + txRightHandDynamic.text + "," + handsTouch;
                            //恢复初始值
                            LhandWave = "W0";
                            LcodeStart.Clear();
                            LcodeEnd.Clear();
            ...
            tx8.text = hand_codes[num2];  //输出最后一个编码
            tx81.text= hand_codesl[num2];
            tx82.text = hand_codesr[num2];

            tx9.text = num2.ToString();
            // num2++;
        }
    }
